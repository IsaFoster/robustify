...
def percentage_shift(df, feature_names, percentage):
    add = (1+(percentage*0.1))
    for feature_name in feature_names:
        df[feature_name] = df[feature_name] * add
    return df

def flip_sign(df, feature_names):
    for feature_name in feature_names:
        df[feature_name] = df[feature_name] * (-1)
    return df

def gaussian_noise(df, feature_names, level):
    print("adding gaussian noise for {} with levels: {}".format(feature_names, level))

def add_or_subtract(df, feature_names, level):
...
def filter_on_method(df, method, feature_names, level=None):
    switcher = {
        'percentageShift': lambda: percentage_shift(df, feature_names, level),
        'flipSign': lambda: flip_sign(df, feature_names),
        'gaussianNoise': lambda: gaussian_noise(df, feature_names, level),
        'addOrSubtract': lambda: add_or_subtract(df, feature_names, level)
    }
    return switcher.get(method, lambda: print("Invalid corruption method for feature {}".format(feature_names)))()


'***********************************************'

def noiseCorruptions_3(df, X_test, y_test, model, corruption_dict, random_state=None, corruptions=10, plot=True):
    for version in tqdm(list(corruption_dict.items()), desc="Total progress: ", position=0):
        parameter_values = []
        accuracy_values = []
        feature_variance = []
        ...
        method = version[0]
        if (isinstance(version[1], dict)):
            feature_names = list(version[1].keys())[0]
            levels = list(version[1].values())[0]
        elif (isinstance(version[1], list) and len(version[1]) == 1):
            feature_names = version[1][0]
            levels = None
        elif (isinstance(version[1], list)):
            feature_names = version[1][0]
            levels = version[1][1]
        else:
        ...
        for level in levels: 
            ...
            for _ in pbar:
                X, y = sampleData(df, 'data_type', 0.4, random_state)

                # corrupt
                corrupted_noise = filter_on_method(X, method, feature_names, level)
...
'''        for level in version:
            ...
            accuracy_values = []
            feature_variance = []
            pbar = tqdm(range (corruptions), desc="Level: {}".format(level), position=2, leave=False)
            for _ in pbar:
